...
f32 JUTResFont::drawString_size_scale(f32 posX, f32 posY, f32 scaleX, f32 scaleY, const char* string, u32 length, bool visible) {
	f32 temp = posX;

	u32 length_minus_one = length - 1;

	for (s32 i = 0; i < length; i++, string++) {
		s32 b = *(u8*)string;
		if (getFontType() == 1) {
			string++;
			b <<= 8;
			b |= *(u8*)string;
			i++;
		} else if (getFontType() == 2 && b >= 0x80 && i != length_minus_one) {
			string++;
			b <<= 8;
			b |= *(u8*)string;
			i++;
		...
		posX += drawChar_scale(posX, posY, scaleX, scaleY, b, visible);
		visible = true;
	}
