    ...
    : J2DPane(parent, stream, is_ex)
{
	...
	if (is_ex) {
		...
		unk100 = new Texture((ResTIMG*)res.getResource(stream, 'TIMG', nullptr));
		unk104 = new Texture((ResTIMG*)res.getResource(stream, 'TIMG', nullptr));
		unk108 = new Texture((ResTIMG*)res.getResource(stream, 'TIMG', nullptr));
		unk10C = new Texture((ResTIMG*)res.getResource(stream, 'TIMG', nullptr));
...
void J2DWindow::resize(int width, int height)
{
    ...
    JSUTreeIterator<J2DPane> iter(&mPaneTree);
	for (iter = mPaneTree.getFirstChild();
	     iter != mPaneTree.getEndChild(); ++iter) {
		if (iter->getTag() == 0x13 && iter->isConnectParent()) {
            int nw = dw;
            int iterwidth = iter->getWidth();
			nw += iterwidth;
            int nh = dh;
            int iterheight = iter->getHeight();
			nh += iterheight;
