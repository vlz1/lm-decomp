...
void J2DScreen::set(JSURandomInputStream* stream) {
    makeHiearachyPanes(this, stream, true, true, false, nullptr);
}
	...
	while (true) {
		if (is_ex) {
			...
			if (stream->read(&magic, 4) != 4) {
				OSPanic("J2DScreen.cpp", 143, "SCRN resource is broken.\n");
			}
			u32 size;
			if (stream->peek(&size, 4) != 4) {
				OSPanic("J2DScreen.cpp", 148, "SCRN resource is broken.\n");
			}
			stream->skip(-4);
			*out_next_pane_offset = size + stream->getPosition();
			...
			if (stream->peek(&tag, 2) != 2) {
				OSPanic("J2DScreen.cpp", 197, "SCRN resource is broken.\n");
			}
			switch (tag) {
			case 0:
				return;
...
{
	OSPanic(__FILE__, 268, "There is a unknown pane in SCRN resource\n");
	return nullptr;
}
...
{
	OSPanic(__FILE__, 295, "There is a unknown pane in SCRN resource\n");
	return nullptr;
}
