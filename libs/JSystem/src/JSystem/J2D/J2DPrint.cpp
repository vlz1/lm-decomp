...
J2DPrint::J2DPrint(JUTFont* font, int param_4, int param_5) {
    private_initiate(font, param_4, param_5,
...
char* J2DPrint::setBuffer(u32 size)
{
	...
	mStrBuff     = new (JKRGetSystemHeap(), 4) char[size];
	mStrBuffSize = size;
	mHeapFlag    = true;
	return old;
}
...
float J2DPrint::getHeight(const char* format, ...)
{
	va_list args;
	va_start(args, format);
...
u16 J2DPrint::doEscapeCode(const u8** param_1, u8 opacity)
{
    ...
    *param_1 += 2;
	...
	case 'CC': {
		unk8       = getNumber(param_1, unk3C, unk8, 16);
		local_40   = unk8;
	...
	case 'GC': {
		unkC       = getNumber(param_1, unk40, unkC, 16);
		local_44   = unkC;
		local_40.a = local_40.a * opacity / 0xff;
		local_44.a = local_44.a * opacity / 0xff;
