...
{
	if (!sDecompObject) {
		sDecompObject = new (JKRHeap::getSystemHeap(), 4) JKRDecomp(priority);
	}
...
{
	JKRDecompCommand* command
	    = prepareCommand(srcBuffer, dstBuffer, srcLength,callback);
	sendCommand(command);
	return command;
}
...
{
	JKRDecompCommand* command
	    = orderAsync(srcBuffer, dstBuffer, srcLength, 0, nullptr);
